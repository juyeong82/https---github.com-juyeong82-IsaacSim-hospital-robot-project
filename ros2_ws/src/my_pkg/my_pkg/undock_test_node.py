...
# [설정]
TEST_REVERSE_DIST = 2.0   # 후진 거리 (m)
TARGET_ANGLE_DEG = 10.0   # 목표 회전 각도 (도)
P_GAIN = 4.0              # 회전 속도 게인
MAX_ROT_SPEED = 0.5       
MIN_ROT_SPEED = 0.1       
REVERSE_SPEED = -0.2     # 후진 속도 (조향과 동시에 하므로 약간 낮춤)
...
class UndockTestNode(Node):
    def __init__(self):
        ...
        self.current_odom_yaw = None  # 현재 로봇의 절대 각도(Odom 기준)
    ...
    def run_undock_test(self, side):
        ...
        # =========================================================
        # [수정됨] Phase 3: 90도 회전 (Odometry Feedback Control)
        # =========================================================
        self.get_logger().info("🔄 Phase 3: Final 90 Degree Turn (Odom)...")
        time.sleep(0.5)

        # 오도메트리 데이터 대기
        wait_cnt = 0
        while self.current_odom_yaw is None and wait_cnt < 10:
            time.sleep(0.1)
            wait_cnt += 1
            
        if self.current_odom_yaw is None:
            self.get_logger().error("❌ No Odom data! Skipping turn.")
        else:
            # 1. 초기 각도 저장
            start_yaw = self.current_odom_yaw
            target_rad = 3.0  # 170도 (라디안)
            target_deg = math.degrees(target_rad) # 로그 출력용 변환
            
            # 2. 회전 방향 설정 (L: 좌회전+, R: 우회전-)
            rot_sign = 1.0 if side == 'L' else -1.0
            cmd = Twist()
            cmd.angular.z = 0.5 * rot_sign  # 회전 속도

            # 3. 오도메트리 보며 회전
            turn_start_time = self.get_clock().now()
            
            while rclpy.ok():
                # 타임아웃 (안전을 위해 10초 지나면 강제 종료)
                if (self.get_clock().now() - turn_start_time).nanoseconds / 1e9 > 30.0:
                    self.get_logger().warn("⏰ Turn Timeout!")
                    break

                # 현재 각도와 시작 각도의 차이 계산 (Wrap-around 처리 포함)
                diff = self.current_odom_yaw - start_yaw
                
                # -PI ~ +PI 범위 정규화 (atan2 사용)
                diff = math.atan2(math.sin(diff), math.cos(diff))
                
                current_moved = abs(diff) # 움직인 각도(절대값)
        ...
        self.stop_robot()
        self.get_logger().info("✅ Final Turn Complete.")
